...
const createAssessment = async ({ courseId, title, description, assessmentType, totalMarks, weightPercentage, dueDate, createdBy, pdfUrl }) => {
  const query = `
    INSERT INTO assessments (course_id, title, description, assessment_type, total_marks, weight_percentage, due_date, created_by, pdf_url)
    VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)
    RETURNING *
  `;
  const { rows } = await pool.query(query, [courseId, title, description, assessmentType, totalMarks, weightPercentage, dueDate, createdBy, pdfUrl]);
  return rows[0];
};
