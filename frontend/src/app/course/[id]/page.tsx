...
export default function CourseDetailPage() {
  ...
  const loadData = async () => {
    try {
      const [courseRes, announcementsRes, streamRes, activityRes, quizzesRes, assessmentsRes, participantsRes, assignmentsRes, lecturesRes] = await Promise.all([
        ...
        api.get<Quiz[]>(`/quizzes/course/${params.id}`).catch(() => []),
        api.get<Assessment[]>(`/assessments/course/${params.id}`).catch(() => []),
        api.get<any[]>(`/courses/${params.id}/enrollments`).catch(() => []),
        api.get<{ assignments: Assignment[] }>(`/assignments/course/${params.id}`).catch(() => ({ assignments: [] })),
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-4xl font-semibold text-white">{course.code} - {course.title}</h1>
              ...
              <Card key={item.stream_id} className="border-white/10 bg-white/5">
                <CardContent className="p-6">
                  <div className="flex items-start justify-between mb-2">
                    <div>
                      <p className="font-semibold text-white">{item.title || 'Update'}</p>
                  ...
                  {item.content && <p className="text-white mt-2">{item.content}</p>}
                </CardContent>
              </Card>
            ))
          )}
